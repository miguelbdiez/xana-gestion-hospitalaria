...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    document.getElementById('messageForm').addEventListener('submit', function(e) {
      ...
      // Example: Send data to a server (uncomment and adjust URL as needed)
      try{
        ...
        .then((data) => {
          if (data.success) {
            //enviar mail por sockets 
            console.log("enviar mail por soket", data.mail)
            socket.emit("mail", {
              mail: data.mail
            });
            window.location.href = `/comunicacion/mail/bandeja`;
