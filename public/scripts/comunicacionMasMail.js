...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  socket.on("mail", (mail) => {
    console.log("Nuevo mail recibido:", mail);
    

// Seleccionamos la lista de mensajes
   const listaMails = document.querySelector(".messages");

// Creamos un nuevo elemento <li> con la estructura del mensaje
    const nuevoMensaje = document.createElement("li");
    nuevoMensaje.classList.add("message-container");

    nuevoMensaje.innerHTML = `
      <!-- Botón de favorito separado del <li> -->
      <span class="action favorite-btn" data-mail-id="${mail.id}">
        <i class="fa ${mail.favorito === 1 ? 'fa-star' : 'fa-star-o'}"></i>
      </span>
      <span class="action favorite-btn" data-mail-id="${mail.id}">
        <i class="fa fa-trash-o"></i>
      </span>

      <!-- El <li> solo contiene el contenido del mensaje -->
      <li class="message ${mail.leido === 0 ? 'unread' : ''}" data-mail-id="${mail.id}">
        <div class="header">
          <span class="from">${mail.emisorMail}</span>
          <span class="date">
            <span class="fa fa-paper-clip"></span>
            ${new Date(mail.fecha).toLocaleString('es-ES', { dateStyle: 'medium', timeStyle: 'short' })}
          </span>
        </div>
        <div class="title">
          ${mail.asunto}
        ...
        <div class="description">
          ${mail.contenido.length > 100 ? mail.contenido.substring(0, 100) + '...' : mail.contenido}
        </div>
      </li>
    `;

    // Insertar el nuevo mensaje al principio de la lista
    listaMails.prepend(nuevoMensaje);
  ...
  messagesList.addEventListener('click', (event) => {
    const messageItem = event.target.closest('.message');
    if (messageItem) {
        const mailId = messageItem.getAttribute('data-mail-id');
        console.log(`Mail ID clicked: ${mailId}`);
        window.location.href = `/comunicacion/mail/verMail?mailId=${mailId}`;

    }
});
     
       //Eventos botones de mail
        favoriteButtons.forEach(button => {
          ...
          // Verificar qué ícono fue clickeado
          if (icon.classList.contains('fa-star') || icon.classList.contains('fa-star-o')) {
    
            // Evento para los botones de favorito
            let icono="";
            if (icon.classList.contains('fa-star')) {
                ...
                icono = true;
                  ...
                  // Obtener el mailId directamente del botón
              const mailId = button.getAttribute('data-mail-id'); 
              console.log(`Mail ID: ${mailId} - Nuevo estado favorito`);
    
              socket.emit("cambiarEstadoMailFav", {
                  icon: icono,
                  id: mailId
                });
                
          } else if (icon.classList.contains('fa-trash-o') ) {
            // Evento para los botones de Basura
            const confirmar = confirm('¿Estás seguro de eliminar este mensaje?');
            
            if (confirmar) {
                // Si el usuario confirma, eliminar de la lista
                const messageContainer = button.closest('.message-container');
                        if (messageContainer) {
                            messageContainer.remove();
                        }            
    
              const messageToDelete = document.querySelector(`.message[data-mail-id="${mailId}"]`);
              if (messageToDelete) {
                messageToDelete.remove(); // Elimina solo el <li class="message">
              }
                // Emitir evento por socket
    
              socket.emit("cambiarEstadoMailTrash", {
                icon: true,
                id: mailId
                });
